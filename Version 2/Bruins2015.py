...
#Download File
def download_file():
    BruinsPath = r'C:\Programming\Bruins2015'
    os.chdir(BruinsPath)
...
text = open('Games.txt', 'w+')
statInfoStart = os.stat('Games.txt')
print "Start: ", statInfoStart.st_size
...
for line in read_file:
    ...
    #Only reads up to today, inclusive
    if date_object > today:
        break
        
text.close()

with open('Games.txt', 'rb') as readText:
    statinfoEnd = os.stat('Games.txt')
    print "End size: ", statinfoEnd.st_size
    
    first = readText.readline()
    readText.seek(-2,2)
    while readText.read(1) != '\n':
        readText.seek(-2,1)
        
    #Removes newline character to prevent adding an entry with no title to the calendar   
    last = readText.readline().rstrip('\n')

#Adds last line of 'Games.txt' to calendar    
# service.events().quickAdd(calendarId=calendarId, text = last).execute()
